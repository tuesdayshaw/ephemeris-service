...
- `API_KEY`: API key for `/v1/*` routes; empty disables auth
- `CACHE_DIR`: directory for snapshot SQLite cache
- `TZ`: default timezone for daily snapshot and moon aspects (default `America/Chicago`)
...
- Mount a persistent disk for ephemeris files/cache (or provide an attached volume path for `EPHE_PATH`).
